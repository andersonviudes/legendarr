...
def resolve_subtitle_provider_chain(session: Session) -> list[SubtitleProvider]:
    ...
    implementation (today: just `opensubtitles`), `id` ascending. An empty list means
    nothing usable is configured — callers log and skip, this is never treated as an
    error. Same shape as `subtitle_translation.provider_chain.resolve_provider_chain`,
    ready for 0.11.0's "second and third `SubtitleProvider` implementation" to extend
    `_PROVIDER_CLASSES` without changing this function.
    """
