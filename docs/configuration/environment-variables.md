...
| `LEGENDARR_PUBLIC_URL` | *(empty)* | The externally-reachable address of this legendarr instance, e.g. `https://legendarr.example.com`. Used to build the Radarr/Sonarr webhook URL shown on the Arr Services settings page. Leave empty to show the relative webhook path with a hint instead. Same `config.yaml` precedence as `LEGENDARR_RADARR_URL`. Also editable from the web UI (Arr Services page). |
